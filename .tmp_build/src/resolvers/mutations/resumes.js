...
export default (t) => {
    t.field('createResume', {
        ...
        resolve: async (parent, args, ctx) => {
            ...
            const resumeSkills = findKeywords(resumeText, allSkills.map((skill) => skill.name)).filter((skill) => skill !== '');
            const filteredSkills = allSkills.filter((skill) => resumeSkills.includes(skill.name));
