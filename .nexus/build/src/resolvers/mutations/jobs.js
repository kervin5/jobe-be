...
export default (t) => {
    t.field('createJob', {
        ...
        resolve: async (parent, args, ctx) => {
            ...
            const locationExists = (await ctx.db.location.findMany({ where: jobLocation })).length > 0;
            let location = {};
            if (locationExists) {
            ...
            const job = await ctx.db.job.create({
                data: {
                    ...
                    skills: {
                        connect: args.skills.map((skill) => ({ id: skill })),
                    },
                    status: 'DRAFT',
                    author: { connect: { id: authorId } },
                    location,
                    branch: { connect: { id: (_a = user === null || user === void 0 ? void 0 : user.branch) === null || _a === void 0 ? void 0 : _a.id } },
    ...
    t.field('updateJob', {
        ...
        resolve: async (parent, args, ctx, info) => {
            var _a;
            let authorId = ctx.request.user.id;
            let JobDataToUpdate = { ...args.data }; //TODO: Implement interface to define payload of object
            if (JobDataToUpdate.author &&
                ...
                authorId = job === null || job === void 0 ? void 0 : job.author.id;
            }
            const jobs = await ctx.db.job.findMany({
                where: {
                    id: args.where.id,
            ...
            if (jobs.length > 0 ||
                ...
                if (JobDataToUpdate.location) {
                    const locationExists = (await ctx.db.location.findMany({
                    ...
                    if (locationExists) {
                        ...
                        const fetchedLocation = await fetchLocation(JobDataToUpdate.location);
                        JobDataToUpdate.location = {
                            create: {
                                name: JobDataToUpdate.location,
                                longitude: fetchedLocation.center[1],
                ...
                if (JobDataToUpdate.categories) {
                    JobDataToUpdate.categories = {
                        set: JobDataToUpdate.categories.map((category) => ({
                            id: category,
                        })),
                    };
                }
                if (JobDataToUpdate.skills) {
                    JobDataToUpdate.skills = {
                        set: JobDataToUpdate.skills.map((skill) => ({ id: skill })),
                    };
                }
                ...
                JobDataToUpdate['branch'] = { connect: { id: (_a = user === null || user === void 0 ? void 0 : user.branch) === null || _a === void 0 ? void 0 : _a.id } };
